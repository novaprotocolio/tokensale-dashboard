...
export class LoginComponent extends BaseComponent implements OnInit {
  ...
  onLogin(formParams, isValidForm) {
    if (isValidForm) {
      ...
      formParams.password = this.commonFunctions
        .encryptPassword(formParams.password)
        .toString();
      this.apiManager.postAPI(API.Login, formParams).subscribe(response => {
        if (response.payload.data.token) {
          ...
          // save token immediately not check web3
          this.sharedService.setToken(response.payload.data.token);
          this.checkNovaAccountBalance(
            '0x' + this.sharedService.getWalletAddress.address,
            response
          );
        } else {
          this.activeForm = 2;
  ...
  checkNovaAccountBalance(account, response) {
    ...
    this.web3Service.getTokenBalance(account).subscribe(
      result => {
        ...
        if (result == '0.00000') {
          this.router.navigate(['/' + RouteConstants.GET_ETHERIUM]);
        } else this.router.navigate(['/' + RouteConstants.TRANSACTION_HISTORY]);
      },
      error => {
        this.sharedService.setLoader(false);
        this.router.navigate(['/' + RouteConstants.GET_ETHERIUM]);
