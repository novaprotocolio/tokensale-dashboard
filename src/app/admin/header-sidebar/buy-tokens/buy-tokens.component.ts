...
export class BuyTokensComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  buyToken(value) {
    ...
    this.web3Service.signedTransaction(gasObj).subscribe(result=> {
      //console.log(result);
      this.sharedService.setLoader(false);
      this.toastr.success(this.translateService.instant('NovaBoughtSuccess'));
      this.createBuyTokenForm();
    }, error=> {
      this.sharedService.setLoader(false);
      this.toastr.error(this.translateService.instant('NovaBoughtFailed'));
      //console.log(error);
    });
  }
  ...
  distributionCalculation(type, isValid, etherToken) {
    if (isValid && etherToken) {
      ...
      if (type == 1) {//Ether Count
        ...
        return this.balance = this.tokenCount + ' ' + this.translateService.instant('SYM');
      }
    } else {
      return this.balance = '';
    }
  ...
  saveTransaction(token, amount, hash) {
    let body = {
      ...
      "description": "Converted " + amount + " ETH to " + this.tokenCount + " "+this.translateService.instant('SYM')+" Completed.",
      "status": "Pending",
      "amount": "+" + this.tokenCount + " " + this.translateService.instant('SYM'),
  ...
  onVerifyPassword(pass, value) {
    this.buyNovaToken(pass, value.ether);
  }
