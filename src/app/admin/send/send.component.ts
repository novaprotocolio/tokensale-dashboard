...
export class SendComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.checkNovaAccountBalance(this.address);
  }
    ...
    this.sendForm = this.fb.group(
      {
        ...
        amount: new FormControl('', [
          <any>Validators.pattern(CommonRegexp.NUMERIC_FLOAT_REGEXP)
        ])
      },
  ...
  sendBalance(address, value, password) {
    ...
    this.web3Service.signedTransaction(gasObj).subscribe(
      result => {
        this.sharedService.setLoader(false);
  ...
  getEtherPrice() {
    this.apiManager.getLocalAPI(API.ETH_PRICE).subscribe(
      res => {
  ...
  getGas() {
    this.web3Service.getGasPrice().subscribe(
      res => {
        this.gasPrice = res;
        // console.log(res);
      },
  ...
  updatePrice(currency, isValid, value) {
    if (isValid && value) {
      if (currency == '0') {
        ...
        this.transactionFee =
          this.web3Service.weiToEtherConvert(this.gasPrice * 198806) + ' Eth';
      } else {
  ...
  transferToken(address, value, password) {
    ...
    this.web3Service.signedTransaction(gasObj).subscribe(
      result => {
        this.sharedService.setLoader(false);
        this.toastr.success(
          this.translateService.instant('tokenTransferSuccess')
        );
        this.createSendForm();
  ...
  saveTransaction(currency, address, amount, hash) {
    let body = {
      ...
      description:
        'Sent ' + amount + ' ' + currency + ' to ' + address + ' Completed.',
      status: 'Pending',
  ...
  show(hash) {
    ...
    this.msgs.push({
      ...
      detail:
        '<a class="hasMsg" href="https://etherscan.io/tx/' +
        hash +
        '" target="_blank">' +
        hash +
        '</a>'
  ...
  checkAccountBalance(account) {
    this.web3Service.getEtherAccountBalance(account).subscribe(
      result => {
  ...
  checkNovaAccountBalance(account) {
    this.web3Service.getTokenBalance(account).subscribe(
      result => {
  ...
  getTime() {
    // this.checkTime = true;
    this.web3Service.getCurrentTimeStamp().subscribe(
      response => {
